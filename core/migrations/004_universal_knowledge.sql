...
-- Migration 004: structured machine-authored knowledge and durable agent state.
--
-- Canonical ownership is deliberately split by source:
--   * Markdown/YAML remains canonical for human-authored notes/memories.
--   * 80-agent-runs/knowledge.journal.jsonl is canonical for machine-authored
--     structured knowledge and agent state.
--   * SQLite is a rebuildable projection/index of both.
--
-- Memory class and kind use the same vocabulary introduced by migration 003:
--   class = working | episodic | semantic | procedural
--   kind  = observation | episode | fact | preference | decision | procedure |
--           constraint | summary (optional for machine-authored records)
