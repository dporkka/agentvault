...
func TestRunMigrations(t *testing.T) {
	...
	// Verify tables exist
	tables := []string{"files", "notes", "tags", "entities", "links", "conversations", "memory_supersessions", "provenance_records", "objects", "object_relations", "memory_records", "agent_sessions", "session_events", "mutation_proposals", "episodes", "temporal_facts", "schema_migrations"}
...
func TestEmbeddedMigrationsPresent(t *testing.T) {
	...
	for _, name := range []string{"001_init.sql", "002_conversations.sql", "003_memory_semantics.sql", "004_universal_knowledge.sql", "005_transactional_mutations.sql", "006_temporal_facts.sql"} {
		if !found[name] {
...
func TestRunInlineMigrations(t *testing.T) {
	...
	tables := []string{"files", "notes", "tags", "entities", "links", "chunks", "conversations", "conversation_messages", "memory_supersessions", "provenance_records", "objects", "object_relations", "memory_records", "agent_sessions", "session_events", "mutation_proposals", "episodes", "temporal_facts", "schema_migrations"}
	for _, table := range tables {
		var name string
		err := db.QueryRow("SELECT name FROM sqlite_master WHERE type='table' AND name=?", table).Scan(&name)
		if err != nil {
	...
	// Verify latest migration version was recorded.
	var version int
	err = db.QueryRow("SELECT MAX(version) FROM schema_migrations").Scan(&version)
	if err != nil {
		t.Fatalf("Failed to query schema_migrations: %v", err)
...
func TestRunMigrationsInlineFallback(t *testing.T) {
	...
	if version != 6 {
		t.Errorf("Expected inline migration version 6, got %d", version)
	}
}
