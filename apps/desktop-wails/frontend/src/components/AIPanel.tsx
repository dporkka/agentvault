...
export default function AIPanel({ onClose, onOpenNote }: Props) {
  ...
  return (
    <div className="w-96 flex flex-col bg-bg-secondary border-l border-border">
      {/* Header */}
      ...
      <div className="flex-1 overflow-auto px-4 py-3 space-y-4">
        {messages.length === 0 && (
          <div className="text-center py-8">
            <Sparkles className="w-8 h-8 mx-auto mb-3 text-accent opacity-50" />
            ...
            <div className="mt-3 space-y-1.5">
              {[
                ...
                <button
                  key={q}
                  onClick={() => setInput(q)}
                  className="block w-full text-xs text-left px-3 py-2 rounded-md bg-bg-tertiary text-text-secondary hover:text-text-primary hover:bg-bg-hover transition-colors"
                >
                  {q}
                </button>
              ))}
              ...
              <div className="space-y-3">
                ...
                {msg.answer?.sources && msg.answer.sources.length > 0 && (
                  <div className="border-t border-border pt-2">
                    <div className="text-xs font-medium text-text-muted mb-1.5">Sources</div>
                    <div className="space-y-1">
                      {msg.answer.sources.map((source: Source, i: number) => (
                        <button
                          key={i}
                          onClick={() => onOpenNote(source.path)}
                          className="w-full flex items-start gap-2 px-2 py-1.5 rounded text-left hover:bg-bg-hover transition-colors"
                        >
                          <FileText className="w-3 h-3 mt-0.5 text-text-muted flex-shrink-0" />
